...
//    vec2 pos = vec2(st*7.);
    ...
    for (int i =0; i<24; i+=1) {
        vec3 pos  = texture(data,vec2(i,time)).rgb; // x is pos,y is time
        vec3 pos2 = texture(data,vec2(i,mod(time+1,30))).rgb; // x is pos,y is time
        vec3 vel = (pos2-pos)*scale;
        float poscolormap = max(0,1-0.005*length(st-pos.xy));
...
//    color.g += scale*(10-length(st-pos.xy));
    outputColor = (0.01)*color+0.99*texture(tex0,st);
}
