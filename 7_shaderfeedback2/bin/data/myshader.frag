...
//    vec2 tvec = vec2(noise(vec2(time+3000,1)),noise(vec2(time+3000,1000)))*2;
    float amnt=0;
    for (int i=0; i<5; i++) {
        float xmove = (noise(vec3(st/11.,time*0.1      ))-0.5)*pow(sin(time*3.03)*0.5+0.5,5);
        float ymove = (noise(vec3(st/11.,time*0.1+3000))-0.5)*pow((-sin(time*3.03))*0.5+0.5,5);
        vec2 move = vec2(xmove,ymove);
    ...
    color/=amnt;
    color = pow(color,vec4(1.005,1.005,1.005,1.005));
